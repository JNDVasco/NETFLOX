...
#
# Ficheiro: userMenus.py
#
# Descrição:
# Este ficheiro contém todos os menus relacionados com o user
#
# Colors
# Main:        palegreen1
# Secondary:   lightcyan
...
# Status: Done
def mainMenu(username, balance, unreadMessages):
...
# Status: Done
def listarArtigos(cursor, dbCon, idUser):
    # Always clear the screen first
...
# Recieves a database cursor a database conection and the user id idUser
# Status: Done
def pesquisarArtigos(cursor, dbCon, idUser):
    # Always clear the screen first
    command = "SELECT pessoa_nome FROM cliente WHERE pessoa_id_pessoa = %s"
    cursor.execute(command, (idUser,))
    username, = cursor.fetchone()
...
# Recieves a database cursor a database conection and the user id idUser
# Status: Done
def listarArtigosAlugados(cursor, dbCon, idUser):
    # Always clear the screen first
    command = "SELECT pessoa_nome FROM cliente WHERE pessoa_id_pessoa = %s"
    cursor.execute(command, (idUser,))
    username, = cursor.fetchone()
...
# Recieves a database cursor a database conection and the user id idUser
# Status: Not Done
def pesquisarArtigosAlugados(cursor, dbCon, idUser):
    # Always clear the screen first
    command = "SELECT pessoa_nome FROM cliente WHERE pessoa_id_pessoa = %s"
    cursor.execute(command, (idUser,))
    username, = cursor.fetchone()
    clearScreen("Pesquisa de Artigos Alugados User V1", username=username)
    # Main Body
    ...
    strToPrint = "Esta funcionalidade está mal implementada"
    print(term.move_xy((term.width // 2) - (len(strToPrint) // 2), borderY + 20) + term.tomato + strToPrint)
    ...
    if option == 1:
        searchType = "Título"
        clearScreen("Pesquisa de Artigos Alugados User V1")
        ...
        clearScreen("Pesquisa de Artigos Alugados User Titulo V1")
        ...
        clearScreen("Pesquisa de Artigos Alugados User Atores V1")
        ...
        clearScreen("Pesquisa de Artigos Alugados User Realizador V1")
        ...
        clearScreen("Pesquisa de Artigos Alugados User Produtor V1")
...
# ==== Historico de artigos alugados ====
# This funtion allows the user to see all the items he rented in the past
# Recieves a database cursor a database conection and the user id idUser
# Status: Done
def verArtigosAlugados(username):
    # Always clear the screen first
...
# Status: Done
def estatisticas(cursor, dbCon, idUser):
    command = "SELECT pessoa_nome FROM cliente WHERE pessoa_id_pessoa = %s"
    cursor.execute(command, (idUser,))
    username, = cursor.fetchone()
...
# Status: Done
def listarArtigosHistorico(cursor, dbCon, idUser):
    # Always clear the screen first
    command = "SELECT pessoa_nome FROM cliente WHERE pessoa_id_pessoa = %s"
    cursor.execute(command, (idUser,))
    username, = cursor.fetchone()
...
def caixaEntrada(cursor, dbConn, idUser):
    command = "SELECT pessoa_nome FROM cliente WHERE pessoa_id_pessoa = %s"
    cursor.execute(command, (idUser,))
    username, = cursor.fetchone()
    ...
    print(term.move_xy((term.width // 2) - 8,
                       borderY + 20) + term.dodgerblue + "Por ler" + term.move_right(2) + term.lightcyan + "Lida")
